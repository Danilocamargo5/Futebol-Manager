...
function Dashboard({ players, teams, matches, setPlayers, rodada, setActiveRodadaId, rodadas }: { players: Player[]; teams: Team[]; matches: Match[]; setPlayers: (value: Player[] | ((old: Player[]) => Player[])) => void; rodada?: Rodada; setActiveRodadaId: (id: string) => void; rodadas: Rodada[] }) {
  ...
  return <div className="animate-rise">
    ...
    <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4"><StatCard label="Jogadores" value={players.length} hint={`${present.length} presentes na rodada`} icon={Users} accent="bg-[#d9f3a1] text-[#456b31]" href="/jogadores" /><StatCard label="Times ativos" value={teams.length} hint="sorteados nesta rodada" icon={Shield} accent="bg-[#f9d995] text-[#835d1b]" href="/times" /><StatCard label="Partidas" value={matches.length} hint="histórico total" icon={ClipboardList} accent="bg-[#f7c9bf] text-[#a34839]" href="/partidas" /><StatCard label="Artilheiro" value={topScorer?.goals ?? 0} hint={topScorer?.p.nickname || 'sem gols ainda'} icon={Goal} accent="bg-[#d7d3f0] text-[#59558c]" href="/rankings" /></div>
    <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]"><section className="rounded-2xl border bg-card p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-muted-foreground">Pulso da pelada</p><h3 className="mt-1 font-display text-2xl font-black uppercase">Quem vem jogar</h3></div><Link href="/jogadores" data-testid="link-dashboard-players" className="text-xs font-bold text-[#487b4f] hover:underline">Ver jogadores <ArrowRight className="ml-1 inline size-3" /></Link></div><p className="text-sm text-muted-foreground">Clique em \"Ver jogadores\" para gerenciar a presença.</p></section><section className="rounded-2xl border bg-[#f0e7d2] p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#7c725f]">Última partida</p><h3 className="mt-1 font-display text-2xl font-black uppercase text-[#253d2e]">{lastMatch ? formatDate(lastMatch.date) : 'Ainda não'}</h3></div><Trophy className="size-6 text-[#ba8a2f]" /></div>{lastMatch ? <><div className="my-7 flex items-center justify-around text-center"><div><span className="mx-auto mb-2 grid size-9 place-items-center rounded-full bg-[#f97316] text-[10px] font-black text-white">{initials(lastMatch.teamA)}</span><p className="max-w-[90px] text-xs font-bold text-[#334d3a]">{lastMatch.teamA}</p></div><div className="font-display text-4xl font-black text-[#253d2e]">{lastMatch.scoreA}<span className="mx-2 text-xl text-[#988e79]">×</span>{lastMatch.scoreB}</div><div><span className="mx-auto mb-2 grid size-9 place-items-center rounded-full bg-[#5d8f68] text-[10px] font-black text-white">{initials(lastMatch.teamB)}</span><p className="max-w-[90px] text-xs font-bold text-[#334d3a]">{lastMatch.teamB}</p></div></div><div className="border-t border-[#d5c9af] pt-4 text-xs text-[#746a56]"><Award className="mr-1 inline size-3.5 text-[#ba8a2f]" /> Melhor goleiro: <strong>{players.find((p) => p.id === lastMatch.bestKeeper)?.nickname ?? '—'}</strong></div></> : <div className="py-12 text-center text-sm text-[#746a56]">Registre a primeira partida no sorteio.</div>}<Link href="/rankings" data-testid="link-dashboard-ranking" className="mt-5 block text-xs font-bold text-[#487b4f] hover:underline">Abrir rankings <ArrowRight className="ml-1 inline size-3" /></Link></section></div>
  </div>;
}
...
function MatchesPage({ matches, players }: { matches: Match[]; players: Player[] }) {
  const sortedMatches = [...matches].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  
  return <div className="animate-rise">
    <PageHeading eyebrow="histórico de partidas" title="Partidas" description="Todas as partidas registradas no histórico da rodada." />
    {sortedMatches.length > 0 ? (
      <div className="space-y-3">
        {sortedMatches.map((match) => (
          <div key={match.id} className="rounded-2xl border bg-card p-5 sm:p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="text-xs text-muted-foreground font-bold">{formatDate(match.date)}</div>
            </div>
            <div className="flex items-center justify-between">
              <div className="text-right flex-1">
                <p className="font-bold text-sm">{match.teamA}</p>
                <p className="text-xs text-muted-foreground">{match.goalsA.length} gols</p>
              </div>
              <div className="mx-4 font-display text-3xl font-black">{match.scoreA} × {match.scoreB}</div>
              <div className="text-left flex-1">
                <p className="font-bold text-sm">{match.teamB}</p>
                <p className="text-xs text-muted-foreground">{match.goalsB.length} gols</p>
              </div>
            </div>
            {(match.bestKeeper || match.goalsA.length > 0 || match.goalsB.length > 0) && (
              <div className="mt-4 border-t pt-3 text-xs text-muted-foreground space-y-1">
                {match.bestKeeper && <p><strong>Melhor goleiro:</strong> {players.find(p => p.id === match.bestKeeper)?.nickname || '—'}</p>}
                {match.goalsA.length > 0 && <p><strong>Gols {match.teamA}:</strong> {match.goalsA.map(id => players.find(p => p.id === id)?.nickname).filter(Boolean).join(', ')}</p>}
                {match.goalsB.length > 0 && <p><strong>Gols {match.teamB}:</strong> {match.goalsB.map(id => players.find(p => p.id === id)?.nickname).filter(Boolean).join(', ')}</p>}
              </div>
            )}
          </div>
        ))}
      ...
      <EmptyState icon={ClipboardList} title="Sem partidas registradas" text="Registre as partidas na tela de sorteio para vê-las aqui." />
    )}
  </div>;
}
