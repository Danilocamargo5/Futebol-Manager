...
function Dashboard({ players, teams, matches, playerEvents, setPlayers, rodada, setActiveRodadaId, rodadas }: { players: Player[]; teams: Team[]; matches: Match[]; playerEvents: PlayerEvent[]; setPlayers: (value: Player[] | ((old: Player[]) => Player[])) => void; rodada?: Rodada; setActiveRodadaId: (id: string) => void; rodadas: Rodada[] }) {
  const [, setLocation] = useLocation();
  ...
  const topScorer = players.map((p) => ({ player: p, count: golCounts.get(p.id) ?? 0 })).sort((a, b) => b.count - a.count)[0];
  const topAssist = players.map((p) => ({ player: p, count: assistCounts.get(p.id) ?? 0 })).sort((a, b) => b.count - a.count)[0];
  const topKeeper = players.map((p) => ({ player: p, count: defensaCounts.get(p.id) ?? 0 })).sort((a, b) => b.count - a.count)[0];
  ...
  return <div className="animate-rise">
    ...
    <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4"><StatCard label="Jogadores" value={players.length} hint={`${present.length} presentes na rodada`} icon={Users} accent="bg-[#d9f3a1] text-[#456b31]" href="/rodadas" /><StatCard label="Times ativos" value={teams.length} hint="sorteados nesta rodada" icon={Shield} accent="bg-[#f9d995] text-[#835d1b]" href="/times" /><StatCard label="Eventos Registrados" value={playerEvents.length} hint="gols, assists e defesas" icon={Trophy} accent="bg-[#f7c9bf] text-[#a34839]" href="/rankings" /><StatCard label="Quem vem jogar" value={present.length} hint={`de ${players.length} confirmados`} icon={Users} accent="bg-[#d7d3f0] text-[#59558c]" href="/jogadores" /></div>
    {playerEvents.length > 0 && <section className="rounded-2xl border bg-card p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-muted-foreground">Top de cada ranking</p><h3 className="mt-1 font-display text-2xl font-black uppercase">Destaques</h3></div><Link href="/live-scoring" data-testid="link-dashboard-live-scoring" className="text-xs font-bold text-[#487b4f] hover:underline">Live Scoring <ArrowRight className="ml-1 inline size-3" /></Link></div><div className="grid gap-4 sm:grid-cols-3">{topScorer && topScorer.count > 0 && <div className="flex items-center gap-3 rounded-lg bg-[#f8d0c8]/30 p-4"><div className="flex flex-col gap-2 flex-1"><p className="text-[10px] font-bold uppercase text-muted-foreground">🏆 Artilheiro</p><p className="text-xs font-bold">{topScorer.player.nickname}</p><p className="text-[11px] font-black text-[#f97316]">{topScorer.count} gol{topScorer.count > 1 ? 's' : ''}</p></div></div>}{topAssist && topAssist.count > 0 && <div className="flex items-center gap-3 rounded-lg bg-[#f9d995]/30 p-4"><div className="flex flex-col gap-2 flex-1"><p className="text-[10px] font-bold uppercase text-muted-foreground">🎯 Assistência</p><p className="text-xs font-bold">{topAssist.player.nickname}</p><p className="text-[11px] font-black text-[#f97316]">{topAssist.count} assistência{topAssist.count > 1 ? 's' : ''}</p></div></div>}{topKeeper && topKeeper.count > 0 && <div className="flex items-center gap-3 rounded-lg bg-[#d9f3a1]/30 p-4"><div className="flex flex-col gap-2 flex-1"><p className="text-[10px] font-bold uppercase text-muted-foreground">🛡️ Defesas Difíceis</p><p className="text-xs font-bold">{topKeeper.player.nickname}</p><p className="text-[11px] font-black text-[#f97316]">{topKeeper.count} defesa{topKeeper.count > 1 ? 's' : ''}</p></div></div>}</div></section>}
  </div>;
}
...
function RankingCard({ title, subtitle, icon: Icon, color, items, suffix, keeper = false }: { title: string; subtitle: string; icon: typeof Goal; color: string; items: { player: Player; count: number }[]; suffix: string; keeper?: boolean }) {
  return <section className="overflow-hidden rounded-2xl border bg-card"><div className="flex items-center gap-3 border-b p-5"><span className={`grid size-11 place-items-center rounded-xl ${color}`}><Icon className="size-5" /></span><div><h2 className="font-display text-2xl font-black uppercase">{title}</h2><p className="text-xs text-muted-foreground">{subtitle}</p></div></div>{items.length ? <div className="divide-y max-h-[500px] overflow-y-auto">{items.map((item, index) => <div data-testid={`row-ranking-${keeper ? 'keeper' : 'scorer'}-${item.player.id}`} key={item.player.id} className="flex items-center gap-3 px-5 py-3.5"><span className={`font-display text-2xl font-black ${index === 0 ? 'text-[#c08c2d]' : 'text-muted-foreground'}`}>{String(index + 1).padStart(2, '0')}</span><span className="grid size-9 place-items-center rounded-full bg-muted text-[10px] font-black">{initials(item.player.name)}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{item.player.nickname}</p><p className="text-xs text-muted-foreground">{item.player.position}</p></div><div className="text-right"><p className="font-display text-2xl font-black">{item.count}</p><p className="text-[10px] font-bold uppercase text-muted-foreground">{suffix}</p></div>{index === 0 && item.count > 0 && <Award className="size-5 text-[#c08c2d]" />}</div>)}</div> : <EmptyState icon={Trophy} title="Sem números ainda" text="Registre eventos no Live Scoring para este ranking ganhar vida." />}</section>;
}
