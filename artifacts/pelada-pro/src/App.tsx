...
function Dashboard({ players, teams, matches, setPlayers, rodada, setActiveRodadaId, rodadas }: { players: Player[]; teams: Team[]; matches: Match[]; setPlayers: (value: Player[] | ((old: Player[]) => Player[])) => void; rodada?: Rodada; setActiveRodadaId: (id: string) => void; rodadas: Rodada[] }) {
  ...
  return <div className="animate-rise">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]"><section className="rounded-2xl border bg-card p-5 sm:p-6"><div className="flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-muted-foreground">Pulso da pelada</p><h3 className="mt-1 font-display text-2xl font-black uppercase">Quem vem jogar</h3></div><Link href="/jogadores" data-testid="link-dashboard-players" className="text-xs font-bold text-[#487b4f] hover:underline">Ver jogadores <ArrowRight className="ml-1 inline size-3" /></Link></div></section><section className="rounded-2xl border bg-[#f0e7d2] p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#7c725f]">Última partida</p><h3 className="mt-1 font-display text-2xl font-black uppercase text-[#253d2e]">{lastMatch ? formatDate(lastMatch.date) : 'Ainda não'}</h3></div><Trophy className="size-6 text-[#ba8a2f]" /></div>{lastMatch ? <><div className="my-7 flex items-center justify-around text-center"><div><span className="mx-auto mb-2 grid size-9 place-items-center rounded-full bg-[#f97316] text-[10px] font-black text-white">{initials(lastMatch.teamA)}</span><p className="max-w-[90px] text-xs font-bold text-[#334d3a]">{lastMatch.teamA}</p></div><div className="font-display text-4xl font-black text-[#253d2e]">{lastMatch.scoreA}<span className="mx-2 text-xl text-[#988e79]">×</span>{lastMatch.scoreB}</div><div><span className="mx-auto mb-2 grid size-9 place-items-center rounded-full bg-[#5d8f68] text-[10px] font-black text-white">{initials(lastMatch.teamB)}</span><p className="max-w-[90px] text-xs font-bold text-[#334d3a]">{lastMatch.teamB}</p></div></div><div className="border-t border-[#d5c9af] pt-4 text-xs text-[#746a56]"><Award className="mr-1 inline size-3.5 text-[#ba8a2f]" /> Melhor goleiro: <strong>{players.find((p) => p.id === lastMatch.bestKeeper)?.nickname ?? '—'}</strong></div></> : <div className="py-12 text-center text-sm text-[#746a56]">Registre a primeira partida no sorteio.</div>}<Link href="/rankings" data-testid="link-dashboard-ranking" className="mt-5 block text-xs font-bold text-[#487b4f] hover:underline">Abrir rankings <ArrowRight className="ml-1 inline size-3" /></Link></section></div>
  </div>;
}
