...
function Dashboard({ players, teams, matches, setPlayers, rodada, setActiveRodadaId, rodadas }: { players: Player[]; teams: Team[]; matches: Match[]; setPlayers: (value: Player[] | ((old: Player[]) => Player[])) => void; rodada?: Rodada; setActiveRodadaId: (id: string) => void; rodadas: Rodada[] }) {
  ...
  const scorers = players.map((p) => ({ player: p, count: scorerCounts.get(p.id) ?? 0 })).sort((a, b) => b.count - a.count).slice(0, 3);
  const assists = players.map((p) => ({ player: p, count: assistCounts.get(p.id) ?? 0 })).sort((a, b) => b.count - a.count).slice(0, 3);
  const keepers = players.map((p) => ({ player: p, count: keeperCounts.get(p.id) ?? 0 })).sort((a, b) => b.count - a.count).slice(0, 3);
  ...
  return <div className="animate-rise">
    ...
    <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4"><StatCard label="Jogadores" value={players.length} hint={`${present.length} presentes na rodada`} icon={Users} accent="bg-[#d9f3a1] text-[#456b31]" href="/jogadores" /><StatCard label="Times ativos" value={teams.length} hint="sorteados nesta rodada" icon={Shield} accent="bg-[#f9d995] text-[#835d1b]" href="/times" /><StatCard label="Partidas" value={matches.length} hint="histórico total" icon={ClipboardList} accent="bg-[#f7c9bf] text-[#a34839]" href="/rankings" /><StatCard label="Artilheiro" value={topScorer?.goals ?? 0} hint={topScorer?.p.nickname || 'sem gols ainda'} icon={Goal} accent="bg-[#d7d3f0] text-[#59558c]" href="/rankings" /></div>
    <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]"><section className="rounded-2xl border bg-card p-5 sm:p-6"><div className="mb-6 flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-muted-foreground">Rankings da rodada</p><h3 className="mt-1 font-display text-2xl font-black uppercase">Top 3</h3></div><Link href="/rankings" data-testid="link-dashboard-ranking" className="text-xs font-bold text-[#487b4f] hover:underline">Ver rankings <ArrowRight className="ml-1 inline size-3" /></Link></div><div className="grid gap-6 lg:grid-cols-3"><div><p className="mb-3 text-xs font-bold uppercase text-muted-foreground">Artilheiros</p><div className="space-y-2">{scorers.length > 0 ? scorers.map((item, idx) => <div key={item.player.id} className="flex items-center gap-2 rounded-lg bg-muted/50 p-2"><span className="text-[10px] font-black text-muted-foreground">{String(idx + 1).padStart(2, '0')}</span><span className="grid size-7 place-items-center rounded-full bg-[#f8d0c8] text-[10px] font-black text-[#a34839]">{initials(item.player.name)}</span><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{item.player.nickname}</p><p className="text-[11px] font-black text-[#f97316]">{item.count} gols</p></div></div>) : <p className="text-xs text-muted-foreground">Sem gols ainda</p>}</div></div><div><p className="mb-3 text-xs font-bold uppercase text-muted-foreground">Assistência</p><div className="space-y-2">{assists.length > 0 ? assists.map((item, idx) => <div key={item.player.id} className="flex items-center gap-2 rounded-lg bg-muted/50 p-2"><span className="text-[10px] font-black text-muted-foreground">{String(idx + 1).padStart(2, '0')}</span><span className="grid size-7 place-items-center rounded-full bg-[#f9d995] text-[10px] font-black text-[#835d1b]">{initials(item.player.name)}</span><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{item.player.nickname}</p><p className="text-[11px] font-black text-[#f97316]">{item.count} assist</p></div></div>) : <p className="text-xs text-muted-foreground">Sem assistências</p>}</div></div><div><p className="mb-3 text-xs font-bold uppercase text-muted-foreground">Goleiros</p><div className="space-y-2">{keepers.length > 0 ? keepers.map((item, idx) => <div key={item.player.id} className="flex items-center gap-2 rounded-lg bg-muted/50 p-2"><span className="text-[10px] font-black text-muted-foreground">{String(idx + 1).padStart(2, '0')}</span><span className="grid size-7 place-items-center rounded-full bg-[#d9f3a1] text-[10px] font-black text-[#456b31]">{initials(item.player.name)}</span><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{item.player.nickname}</p><p className="text-[11px] font-black text-[#f97316]">{item.count} destaque</p></div></div>) : <p className="text-xs text-muted-foreground">Sem destaques</p>}</div></div></div></section><section className="rounded-2xl border bg-card p-5 sm:p-6"><div className="flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-muted-foreground">Pulso da pelada</p><h3 className="mt-1 font-display text-2xl font-black uppercase">Quem vem jogar</h3></div><Link href="/jogadores" data-testid="link-dashboard-players" className="text-xs font-bold text-[#487b4f] hover:underline">Ver jogadores <ArrowRight className="ml-1 inline size-3" /></Link></div></section></div>
  </div>;
}
...
function RankingsPage({ players, matches }: { players: Player[]; matches: Match[] }) {
  ...
  const assistCounts = new Map<string, number>(); relevant.forEach((match) => [...(match.assistsA || []), ...(match.assistsB || [])].forEach((player) => assistCounts.set(player, (assistCounts.get(player) ?? 0) + 1)));
  ...
  const scorers = players.map((player) => ({ player, count: scorerCounts.get(player.id) ?? 0 })).sort((a, b) => b.count - a.count || a.player.name.localeCompare(b.player.name)); 
  const assists = players.map((player) => ({ player, count: assistCounts.get(player.id) ?? 0 })).sort((a, b) => b.count - a.count || a.player.name.localeCompare(b.player.name));
  const keepers = players.map((player) => ({ player, count: keeperCounts.get(player.id) ?? 0 })).sort((a, b) => b.count - a.count || a.player.name.localeCompare(b.player.name));
  return <div className="animate-rise"><PageHeading eyebrow="números que contam a história" title="Rankings" description="Artilheiros, assistência e muralha. Calculados a partir das partidas que você registrou." action={<div className="flex rounded-xl border bg-card p-1">{(['week', 'month', 'year'] as const).map((item) => <button key={item} data-testid={`button-period-${item}`} onClick={() => setPeriod(item)} className={`rounded-lg px-3 py-2 text-xs font-bold ${period === item ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}>{item === 'week' ? 'Semana' : item === 'month' ? 'Mês' : 'Ano'}</button>)}</div>} /><div className="mb-6 flex items-center gap-2 text-xs font-bold text-muted-foreground"><Clock3 className="size-4" /> Exibindo partidas dos últimos {since === 7 ? '7 dias' : since === 30 ? '30 dias' : '12 meses'} · {relevant.length} partida{relevant.length === 1 ? '' : 's'}</div><div className="grid gap-6 lg:grid-cols-3"><RankingCard title="Artilheiros" subtitle="Quem mais balançou a rede" icon={Goal} color="bg-[#f8d0c8] text-[#a34839]" items={scorers} suffix="gols" /><RankingCard title="Assistência para gol" subtitle="Quem mais criou oportunidades" icon={Swords} color="bg-[#f9d995] text-[#835d1b]" items={assists} suffix="assistências" /><RankingCard title="Melhores goleiros" subtitle="Prêmios de muralha" icon={Shield} color="bg-[#d9f3a1] text-[#456b31]" items={keepers} suffix={keepers.length ? 'destaques' : 'destaque'} keeper /></div></div>;
}
